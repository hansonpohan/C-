...
namespace WindowsFormsApp5
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            arrayTempStudentName = new string[5];//先給暫存空間以免之後比較好修改或增加
            arrayTempStudentScore = new int[5];

            lbl成績總和.Text = totalScore(arrayStudentScore).ToString();
            lbl成績平均.Text = totalScore(arrayStudentScore).ToString();
        }        
        ...
        {//sort排序英文A到Z,中文按筆畫
            ...
            //
            //
            string strMsg = "";//先給一個空字串後面在加入其他字串
        ...
        {
            Array.Sort(myArrayScore, myArrayName);//key-value sort第一個參數先排序完第二個參數跟著第一個參數的排序
            //
            //
            string strMsg = "";
            for (int idx = myArrayName.GetUpperBound(0); idx < myArrayName.Length; idx -= 1)//idx -= 1由大到小
            {
        ...
        {
            if (txt姓名搜尋.Text.Length > 0)
            {
                int index, num = 0;
                string strMsg = "搜尋結果\n----------------------\n";
                string strSearchName = txt姓名搜尋.Text;

                Array.Copy(arrayStudentName, arrayTempStudentName, arrayStudentName.Length);
                Array.Copy(arrayStudentScore, arrayTempStudentScore, arrayStudentScore.Length);

                成績排序(arrayTempStudentName, arrayTempStudentScore);
                //IndexOf搜尋資料在索引陣列的位置並回傳
